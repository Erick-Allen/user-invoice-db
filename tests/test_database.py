...
class TestDatabaseCRUD(unittest.TestCase):
    ...
    def test_create_user(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
    ...
    def test_get_user_by_id(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        row = database.get_customer_by_id(self.cursor, user_id)
    ...
    def test_get_users(self):
        user_id_1 = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        user_id_2 = database.create_customer(self.cursor, TEST_NAME_2, TEST_EMAIL_2)
        rows = database.get_customers(self.cursor)
    ...
    def test_get_users_filter_by_min_total(self):
        ...
        rows = database.get_customers(self.cursor, min_total_dollars=500)
    ...
    def test_update_user_name_only(self):
        ...
        updated_name = "Timmy"
        user_was_updated = database.update_customer(self.cursor, user_id, name=updated_name)
        updated_user = database.get_customer_by_id(self.cursor, user_id)
    ...
    def test_update_user_email_only(self):
        ...
        user_was_updated = database.update_customer(self.cursor, user_id, email=updated_email)
        updated_user = database.get_customer_by_id(self.cursor, user_id)
    ...
    def test_update_user_name_and_email(self):
        ...
        user_was_updated = database.update_customer(self.cursor, user_id, name=updated_name, email=updated_email)
        updated_user = database.get_customer_by_id(self.cursor, user_id)
    ...
    def test_update_user_no_fields_returns_false(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        self.assertFalse(database.update_customer(self.cursor, user_id))

    def test_delete_user(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)

        user_was_deleted = database.delete_customer(self.cursor, user_id)
        row = database.get_customer_by_id(self.cursor, user_id)
    ...
    def test_get_invoices_by_user_id(self):
        ...
        invoice_id_1 = database.add_invoice_to_user(self.cursor, user_id, "1/20/2025", 300.25)
        invoice_id_2 = database.add_invoice_to_user(self.cursor, user_id, "2/17/2025", 100)
        rows = database.get_invoices_by_customer_id(self.cursor, user_id)
    ...
    def test_get_invoices_by_user_and_range_inclusive(self):
        ...
        end_date = "6/5/2025"
        rows = database.get_invoices_by_customer_and_range(self.cursor, user_id, start_date, end_date)
    ...
    def test_delete_invoice(self):
        ...
        remaining = database.get_invoices_by_customer_id(self.cursor, user_id)
    ...
    def test_delete_user_cascades_invoices(self):
        user_id = database.create_customer(self.cursor, "John", "john@example.com")
        invoice = database.add_invoice_to_user(self.cursor, user_id, "1/1/2025", 100)
        deleted_user = database.delete_customer(self.cursor, user_id)
        self.assertTrue(deleted_user)

        cascaded_invoices = database.get_invoices_by_customer_id(self.cursor, user_id)
    ...
    def test_create_user_empty_name_raises(self):
        with self.assertRaises(ValueError):
            database.create_customer(self.cursor, " ", "john@example.com")

    def test_create_user_invalid_name_raises(self):
        with self.assertRaises(ValueError):
            database.create_customer(self.cursor, "John 2 Doe", "johndoe@example.com")

    def test_create_user_empty_email_raises(self):
        with self.assertRaises(ValueError):
            database.create_customer(self.cursor, "John Doe", " ")

    def test_create_user_invalid_email_raises(self):
        with self.assertRaises(ValueError):
            database.create_customer(self.cursor, "John Doe", "invalid-email")

    def test_create_user_duplicate_name_allowed(self):
        user1_id = database.create_customer(self.cursor, TEST_NAME_1, "first@example.com")
        user2_id = database.create_customer(self.cursor, TEST_NAME_1, "second@example.com")
        self.assertNotEqual(user1_id, user2_id)

    def test_create_user_duplicate_email_raises(self):
        database.create_customer(self.cursor, "John Doe", "same@example.com")
        with self.assertRaises(ValueError):
            database.create_customer(self.cursor, "John Doe", "same@example.com")
    ...
    def test_update_user_duplicate_email_raises(self):
        ...
        with self.assertRaises(ValueError):
            database.update_customer(self.cursor, user_id_2, email=duplicate_email)

    def test_delete_user_invalid_id_returns_false(self):
        self.assertFalse(database.delete_customer(self.cursor, INVALID_USER_ID))
    ...
    def test_get_invoices_by_user_and_range_invalid_start_raises(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        with self.assertRaises(ValueError):
            database.get_invoices_by_customer_and_range(self.cursor, user_id, "invalid-date", "2/20/2025")

    def test_get_invoices_by_user_and_range_invalid_end_raises(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        with self.assertRaises(ValueError):
            database.get_invoices_by_customer_and_range(self.cursor, user_id, "2/20/2025", "invalid-date")


    def test_get_invoices_by_user_id_empty_list(self):
        user_id = database.create_customer(self.cursor, TEST_NAME_1, TEST_EMAIL_1)
        invoices = database.get_invoices_by_customer_id(self.cursor, user_id)
